...
const Notification = () => {
  const [notifications, setNotifications] = useState<NotaficationType[]>([]);
  const [open, setOpen] = useState(false);

  useEffect(() => {
    socket.on("getNotification", (data: NotaficationType) => {
  ...
  const handleClick = (notification: NotaficationType) => {
  ...
  return (
    <div className="relative">
      ...
      >
        <div className="relative">
          <Imag path={`icons/notification.svg`} alt="" w={24} h={24} />
        ...
        <span className="hidden xxl:inline">Notafications</span>
      </div>
      {open && (
